...
import type { TestimonialAuthor, TestimonialsSingleWithAvatarProps } from './types'
...
const DEFAULT_PROPS: TestimonialsSingleWithAvatarProps = {
  quote:
...
export default function TestimonialsSingleWithAvatar({
  quote = DEFAULT_PROPS.quote,
  author = DEFAULT_PROPS.author ?? DEFAULT_AUTHOR,
  className = '',
}: TestimonialsSingleWithAvatarProps = {}) {
  const rootClassName = ['isolate overflow-hidden bg-white px-6 lg:px-8', className].filter(Boolean).join(' ')
      ...
      <div className="relative mx-auto max-w-2xl py-24 sm:py-32 lg:max-w-4xl">
        ...
        <figure className="grid grid-cols-1 items-center gap-x-6 gap-y-8 lg:gap-x-10">
          ...
          <div className="col-end-1 w-16 lg:row-span-4 lg:w-72">
            ...
            <img alt={author?.name ?? ''} src={author?.imageUrl ?? ''} className="rounded-xl bg-indigo-50 lg:rounded-3xl" />
          </div>
          <figcaption className="text-base lg:col-start-1 lg:row-start-3">
            <div className="font-semibold text-gray-900">{author?.name}</div>
            {author?.title ? <div className="mt-1 text-gray-500">{author.title}</div> : null}
          </figcaption>
        </figure>
      </div>
    </section>
...
export type { TestimonialAuthor, TestimonialsSingleWithAvatarProps } from './types'
