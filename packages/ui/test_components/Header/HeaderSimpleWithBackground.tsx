...
import type { HeaderSimpleWithBackgroundProps } from './types'

const DEFAULT_PROPS: Required<Omit<HeaderSimpleWithBackgroundProps, 'className'>> = {
  eyebrow: undefined,
...
export default function HeaderSimpleWithBackground({
  eyebrow = DEFAULT_PROPS.eyebrow,
  title = DEFAULT_PROPS.title,
  description = DEFAULT_PROPS.description,
  background = DEFAULT_PROPS.background,
  className = '',
}: HeaderSimpleWithBackgroundProps = {}) {
  const rootClassName = ['relative isolate overflow-hidden bg-white px-6 py-24 sm:py-32 lg:px-8', className]
    .filter(Boolean)
    .join(' ')
  ...
  return (
    <div className={rootClassName}>
      {background?.imageUrl ? (
        ...
        <img alt={background.imageAlt ?? ''} src={background.imageUrl} className="absolute inset-0 -z-10 size-full object-cover opacity-10" />
      ) : null}
      <div
        aria-hidden="true"
      ...
      <div className="mx-auto max-w-2xl text-center">
        ...
        {description ? (
          <p className="mt-8 text-pretty text-lg font-medium text-gray-700 sm:text-xl/8">{description}</p>
        ) : null}
      </div>
    </div>
  )
...
export type { HeaderSimpleWithBackgroundProps } from './types'
