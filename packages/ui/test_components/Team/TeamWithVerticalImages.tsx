...
import type { TeamMember, TeamWithVerticalImagesProps } from './types'
...
const DEFAULT_PROPS: TeamWithVerticalImagesProps = {
...
export default function TeamWithVerticalImages({
  ...
  className = '',
}: TeamWithVerticalImagesProps = {}) {
  const rootClassName = ['bg-white py-24 md:py-32', className].filter(Boolean).join(' ')
    ...
    <div className={rootClassName}>
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-20 px-6 lg:px-8 xl:grid-cols-5">
        ...
        <ul role="list" className="divide-y divide-gray-200 xl:col-span-3">
          {members.map((member) => (
            <li key={member.name} className="flex flex-col gap-10 py-12 first:pt-0 last:pb-0 sm:flex-row">
...
export type { TeamMember, TeamWithVerticalImagesProps } from './types'
